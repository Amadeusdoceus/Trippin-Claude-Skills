...
- **H12.2** Fila local de operações pendentes (criar/editar feito offline), via camada `TrippinAPI`.
  ...
  - **Status: feito em 2026-08-30, com escopo reduzido registrado.** Cronograma (criar/editar/excluir evento) e Despesas (criar despesa, quitar/reabrir) entram na fila; upload de foto/documento fica **fora** da fila offline — é um binário indo para o Storage, que exige rede de qualquer forma, então enfileirar não faria sentido. Reordenar evento (H6.4, um recurso já adaptado do gesto nativo original) também fica fora: offline, é ignorado silenciosamente em vez de arriscar uma reconciliação de ordem complexa por um ganho pequeno. Editar/excluir um item ainda não sincronizado (criado durante a mesma sessão offline) mescla direto na operação de criação já enfileirada, em vez de gerar uma segunda operação — evita tentar "atualizar" algo que o servidor ainda não conhece. **Ajuste feito junto com H15.1 (2026-08-30):** originalmente, uma falha genuína de sincronização (não só falta de conexão) era descartada da fila silenciosamente; agora ela permanece marcada até o usuário mandar tentar de novo pela tela de Registro de sincronização — mudança necessária para H15.1 fazer sentido, sem a qual não haveria nada para a ação "Tentar novamente" agir sobre.
...
- **H15.1 (lado usuário)** Tela "Registro de sincronização" (Perfil ou config. da Viagem).
  ...
  - **Status: feito em 2026-08-30**, agora que o E12 existe. Implementado dentro da aba Integrantes (o "config. da viagem" da spec), como uma seção "Registro de sincronização" logo abaixo do histórico de auditoria — não é uma tela separada, mas o mesmo lugar de configurações administrativas da viagem. É um registro local do **dispositivo** (mesmo motivo de a fila offline ser local): cada alteração feita offline aparece com horário e descrição ("Criar evento: X", "Quitar despesa", etc.), e o status muda sozinho para "Corrigido automaticamente" quando a sincronização automática ao reconectar dá certo — exatamente a redação do critério. Uma falha genuína (não só falta de conexão) fica marcada como "Falhou" com um botão "Tentar novamente"; diferente de uma pendência por falta de conexão (que continua sincronizando sozinha a cada reconexão), uma falha genuína só é retentada quando o usuário pede, para não martelar repetidamente algo que provavelmente vai falhar de novo. **Mudança de comportamento no E12 para viabilizar isto:** antes, um erro genuíno de sincronização era descartado da fila silenciosamente (só ia para o log de engenharia, H15.2); agora ele permanece na fila, marcado, até o usuário tentar de novo ou ele ser resolvido. Verificado com um teste real de ponta a ponta (criar evento offline → reconectar → sincroniza sozinho → aba Integrantes mostra a entrada com a descrição certa e "Corrigido automaticamente"); esse mesmo teste confirmou que a refatoração da fila de sincronização do E12 não regrediu (fila volta a zero itens depois de reconectar, como antes).
