...
> features medianas.
>
> **Ver `01-VN-revisada.md` §2.4** para uma leitura mais cética desse "fosso": a implementação de
> v1 (parser estruturado, sem LLM — decisão #7 da seção 9) é mais fácil de replicar do que a
> redação acima sugere.
...
>
> **Ver `01-VN-revisada.md` §2.2/§2.3** — a North Star acima ainda não tem definição operacional de
> "concluída", e as metas de 30 dias não têm um motor de aquisição declarado por trás delas.
...
| **Integração com e-mail (Gmail/Outlook)** para localizar, em uma pasta/rótulo indicado pelo usuário, mensagens com reservas e confirmações — sob demanda, sem sincronização em segundo plano | Amplia a inteligência de documentos além do upload manual, mas depende de revisão de escopo restrito do Gmail (Google) e registro de app no Microsoft Graph (Outlook) — prazo de compliance que não deve travar o MVP. Ver `01-VN-revisada.md` §3 | v2 |
...
> futuro.
>
> **Ver `01-VN-revisada.md` §2.5** — a decisão #1 (front em arquivo único) ainda não tem risco
> correspondente na tabela da seção 10, e a superfície só cresce a cada novo pilar (docs,
> hospedagem, e-mail).
...
>
> **Ver `01-VN-revisada.md` §2.6** para riscos de privacidade ainda não cobertos aqui (retenção/
> exclusão de documentos sensíveis) e **§3.5** para os riscos específicos da integração de e-mail.
...
>
> **Ver `01-VN-revisada.md` §4** — mais três perguntas em aberto foram identificadas na revisão
> crítica (modelo de negócio, motor de crescimento, escopo do scan de e-mail).
